...
const TaskController = class TaskController extends TaskDB {
    ...
    static async __CreateNewTask(data:TaskData,file:string){
        try {
            ...
            let createdCard:{id:string}|any = await BoardController.createCardInList(data.listId,data.name,file)
            data.cardId = createdCard.id

            // Check if there is attachment
            let attachment
            if(file){ 
                attachment = await BoardController.createAttachmentOnCard(createdCard.id,file)
            }
            // Add task to DB
            delete data.listId
