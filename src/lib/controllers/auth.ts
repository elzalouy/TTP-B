...
import { comparePassword, createJwtToken, jwtVerify, hashBassword } from './../services/auth/auth';
...
const AuthController = class AuthController extends UserDB {
    ...
    static async __newPasswordSet(token:string,password:string){
        try {
            ...
            let user = await super.updateUser({id:verifyToken.user.id,password:cryptPassword})
