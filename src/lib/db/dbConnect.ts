...
const mongoDB: () => Promise<void> = async () => {
  try {
    ...
    const userInfo:any = await UserDB.findUser({ email: process.env.SUPER_ADMIN_EMAIL })
    if(!userInfo){
      let passwordHash: string = await hashBassword(process.env.SUPER_ADMIN_PASSWORD)
      const data:UserData = {
